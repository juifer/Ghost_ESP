...
// =======================================================
// ADDED: Power Off menu item (no icon needed for now)
// =======================================================
extern void ghost_poweroff(void);
extern const lv_img_dsc_t power_icon_placeholder; 
// If you don't have an icon, we will ignore it safely.
...
static menu_item_t menu_items[] = {
    ...
    // ⭐ NEW POWER OFF MENU ITEM ⭐
    {"Power Off", NULL} 
    // No icon required. LVGL will skip displaying it.
};
...
static void init_menu_colors(void) {
    ...
    menu_items[4].border_color = lv_color_hex(0xFFFFFF); // Power Off Icon Color (white)
}
...
static void update_menu_item(bool slide_left) {
    ...
    if (LV_HOR_RES <= 128 && LV_VER_RES <= 128) btn_size = 80;
    ...
    if (menu_items[selected_item_index].icon != NULL) {
        ...
        int x_pos = (btn_size - icon_size) / 2 - 3;
        int y_pos = (btn_size - icon_size) / 2 - 5;
